...
|--------|--------|-----|------|--------|
| `VOL75_FINAL.set` | Volatility 75 | M15 | 0.50% | 🧪 **PAPER — validation chart, A/B arm A (since 2026-09-04).** `InpLiveExecution=false` + `InpPaperEquity=50.0` + `InpPaperSpreadMult=1.5` (conservative virtual fills). Virtual fills/SL/TP only — zero broker orders; ledger `MitemshubAI_paper_Volatility_75_Index.csv`, telemetry `paper_open`/`paper_close`, watchdog `python scripts/demo_watchdog.py`. **v26.32 strategy: TP 1.8×stop only — the v26.29 static filters (EMA-side veto, pb-min 0.60) are OFF** after the 210-day walk-forward measured them as a ~33R drag (26 folds, 508 trades: TP 1.8 +52.4R t=1.68 vs legacy +51.4R t=1.57; artifact `walkforward_210d.json`). BandFade stays OFF (live spread vetoes 100% of BF entries). Flip `InpLiveExecution=true` ONLY after the paper journal shows positive expectancy over a meaningful sample (README paper-first rule) |
| `VOL75_TP24.set` | Volatility 75 | M15 | 0.50% | 🧪 **PAPER — A/B arm B (added 2026-09-04).** Identical to VOL75_FINAL except `InpTpMult=2.4` (legacy geometry) and `InpMagic=7788100`. The 210-day replay ties TP 1.8 (+52.4R) with TP 2.4 (+51.4R); live paper data adjudicates. **Must run in a DIFFERENT terminal instance than FINAL** — all ledger/state/telemetry files are symbol-tagged, not instance-tagged, so two same-symbol charts in one terminal corrupt each other. Keep `InpPaperEquity=50.0` identical for comparability |

**chart04 landmine — decision: DETACH (2026-09-04).** The live terminal's Default profile has a *second* V75 chart (`chart04.chr`) with a MitemshubAI attached at `InpLiveExecution=true`, `InpTpMult=2.0`, BandFade ON, and **the same magic 7788075 as arm A**. Arm B can never live there (same terminal + same symbol = corrupted ledgers), and a live-execution V75 chart on unvalidated inputs is a funded-account landmine. Resolution: detach the EA from chart04 entirely; arm B runs in a **second terminal instance** instead. Tool: `python scripts/detach_chart_ea.py --scan` (read-only), then with **all MT5 terminals closed**: `python scripts/detach_chart_ea.py --chart "<terminal>\MQL5\Profiles\Charts\Default\chart04.chr"` — refuses while terminal64.exe runs, backs up, strips only the `<expert>` block, verifies. The weekly report (`scripts/paper_weekly.py`) flags this chart until the detach is done |
