//+------------------------------------------------------------------+
//|                                         MitemshubAI_v17_7.mq5    |
//|                     MITEMSHUB AI MARKET ENGINE v17.7              |
//|   V100 Optimized • H1+H4 • Breakout + Pullback + Structure SL     |
//|   Safe Risk Sizing • Early Exit • Session SL                      |
//+------------------------------------------------------------------+
...
input int    InpEarlyExitHours   = 5;
input double InpEarlyExitMinR    = 0.20;
...
input bool   InpTrendOnly        = false;     // v17.7: allows ranging + momentum
input double InpMinEmaSeparation = 0.18;      // v17.7: less selective

input group "=== Entry Signals ==="
input double InpPullbackMin      = 0.35;      // v17.7: less selective
input double InpPullbackMax      = 2.60;      // v17.7: wider range
input int    InpRsiPeriod        = 14;
input double InpRsiBuyMax        = 68.0;      // v17.7: wider RSI band
input double InpRsiSellMin       = 32.0;      // v17.7: wider RSI band
input bool   InpRequireEmaAlign  = false;     // v17.7: faster entries

input bool   InpUseBreakout      = true;
input int    InpBreakoutLookback = 8;         // v17.7: tighter lookback
input double InpBreakoutBuffer   = 0.08;      // v17.7: less buffer

input bool   InpUseMomentum      = true;
input int    InpMomLookback      = 10;
input double InpMomMinMove       = 0.90;      // v17.7: lower threshold
input double InpMomRsiBuy        = 50.0;      // v17.7: wider RSI
input double InpMomRsiSell       = 50.0;      // v17.7: wider RSI
input double InpSlopeThresh      = 0.30;      // v17.7: lower threshold

input group "=== ATR Filter ==="
input int    InpAtrPeriod        = 14;
input int    InpAtrLookback      = 150;
input double InpAtrLowPct        = 6.0;       // v17.7: more permissive
input double InpAtrHighPct       = 92.0;      // v17.7: more permissive

input group "=== Risk & Exits ==="
input double InpRiskPerTrade     = 0.0035;    // 0.35% risk
...
{
   if(!InpUseSessionSL) return 1.70;
...
int OnInit()
{
   ...
   {
      Print("v17.7: Indicator handle failed");
      return INIT_FAILED;
   }
   ...
   PrintFormat("MITEMSHUB AI v17.7 started | TrendOnly=%s | EMA Sep=%.2f | RSI=%d/%d | V100 H1",
               InpTrendOnly?"ON":"OFF", InpMinEmaSeparation, (int)InpRsiSellMin, (int)InpRsiBuyMax);
...
void OnDeinit(const int reason)
{
   ...
   Print("========================================");
   PrintFormat("v17.7 Summary | Trades:%d  WR:%.1f%%  TotalR:%+.2f", g_trades, wr, g_total_r);
...
int GenerateSignal(string &sig_type)
{
   ...
   if(InpUseBreakout)
   {
      ...
      {
         // In ranging, use EMA slope for direction
      ...
      if(dir != 0 && Bars(_Symbol, g_tf_entry) > InpBreakoutLookback + 2)
      {
         ...
         if(dir > 0 && price > hh + buffer && body > 0 && rsi[0] < InpRsiBuyMax)
         {
            sig_type = "BREAKOUT_LONG";
            return 1;
         }
         if(dir < 0 && price < ll - buffer && body < 0 && rsi[0] > InpRsiSellMin)
         {
   ...
   if(g_regime == REGIME_BULLISH || g_regime == REGIME_BEARISH)
   {
      ...
      if(dir > 0 && price > emaF[0] + 0.65 * atr[0]) return 0;
      if(dir < 0 && price < emaF[0] - 0.65 * atr[0]) return 0;
      ...
      if(dir > 0 && body < -0.12 * atr[0]) return 0;
      if(dir < 0 && body >  0.12 * atr[0]) return 0;
...
void OpenTrade(int direction, string sig_type)
{
   ...
   double atr_stop  = stop_mult * atr[0];

   // Structure SL
   double structure_sl = 0;
   if(direction > 0)
   {
      double swing = iLow(_Symbol, g_tf_entry, 1);
      for(int k=2; k<=6; k++) swing = MathMin(swing, iLow(_Symbol, g_tf_entry, k));
      structure_sl = swing - 0.15 * atr[0];
   }
   else
   {
      double swing = iHigh(_Symbol, g_tf_entry, 1);
      for(int k=2; k<=6; k++) swing = MathMax(swing, iHigh(_Symbol, g_tf_entry, k));
      structure_sl = swing + 0.15 * atr[0];
   }

   double struct_dist = MathAbs(entry - structure_sl);
   double stop_dist   = MathMax(atr_stop, struct_dist);

   // Safety limits
   double max_stop = entry * 0.028;
   if(stop_dist > max_stop) stop_dist = max_stop;
   if(stop_dist < atr[0] * 0.45) stop_dist = atr[0] * 0.45;

   double sl = (direction > 0) ? entry - stop_dist : entry + stop_dist;
   double tp = (direction > 0) ? entry + InpAtrTargetMult * stop_dist
                               : entry - InpAtrTargetMult * stop_dist;
   ...
   {
      if(direction > 0)
         ok = trade.Buy(vol, _Symbol, 0, NormalizeDouble(sl, _Digits), NormalizeDouble(tp, _Digits), "MITEM_v17.7");
      else
         ok = trade.Sell(vol, _Symbol, 0, NormalizeDouble(sl, _Digits), NormalizeDouble(tp, _Digits), "MITEM_v17.7");
   }
   else
   {
   ...
   if(!ok)
   {
      PrintFormat("v17.7 Order failed: %d %s", trade.ResultRetcode(), trade.ResultComment());
   ...
   PrintFormat("[v17.7] %s %s @%.5f SL=%.5f TP=%.5f Vol=%.2f Risk=%.2f%%",
               sig_type, direction>0?"BUY":"SELL", entry, sl, tp, vol, InpRiskPerTrade*100);
}
...
void ClosePosition(string reason)
{
   ...
   {
      Print("v17.7 Close failed: ", trade.ResultRetcode());
      return;
   }
   ...
   PrintFormat("[v17.7] CLOSE %s R=%+.3f | TotalR=%+.2f", reason, r_mult, g_total_r);
...
void CreateDashboard()
{
   for(int i=0; i<26; i++)
   {
      dash_names[i] = "M177_" + IntegerToString(i);
...
void UpdateDashboard()
{
   ...
   L[4]  = StringFormat("Session: %s | SL Mult: %.2f", GetSessionStr(), GetCurrentStopMult());
   L[5]  = StringFormat("Trades: %d | WR: %.1f%% | R: %+.2f", g_trades, wr, g_total_r);
   L[6]  = StringFormat("Daily: $%.2f | ConsecL: %d", g_daily_pnl, g_consec_loss);
   L[7]  = StringFormat("Status: %s | CD: %d", g_paused ? "PAUSED" : "ACTIVE", g_cooldown);
   L[8]  = StringFormat("Risk: %.2f%% | Target: %.1fx | Hold: %d", InpRiskPerTrade*100, InpAtrTargetMult, InpHoldBars);
   L[9]  = StringFormat("Trail: %.1f/%.1f | BE: %.1f | Early: %dh", InpTrailStartATR, InpTrailDistATR, InpBETriggerATR, InpEarlyExitHours);
   L[10] = StringFormat("MaxDD: %.2f%% | T/T/E/S: %d/%d/%d/%d", dd, g_target_exits, g_time_exits, g_early_exits, g_stop_exits);
   ...
   if(g_ticket > 0 && PositionSelectByTicket(g_ticket))
   {
      ...
      L[line++] = StringFormat("R now: %+.2f | Held: %.1fh", r_now, hrs);
   }
...
{
   string name = "M177_" + tag + "_" + IntegerToString((int)t);
