...
//|   Single-Symbol • Regime + Pullback + Trailing + Breakeven       |
//|   v16.6: Optimized params, 3-bar cooldown, wider pullback        |
//+------------------------------------------------------------------+
...
input double InpAtrStopMult      = 1.6;       // v16.6: optimized for V100
...
input double InpTrailStartATR    = 0.6;       // v16.6: faster profit lock
input double InpTrailDistATR     = 0.7;       // v16.6: tighter trail
...
int OnInit()
{
   ...
   {
      Print("[MITEM v16.6] Indicator handle creation failed");
      return INIT_FAILED;
   }
   ...
   PrintFormat("[MITEM v16.6] Started | Risk=%.2f%% | Symbol=%s | Magic=%d | Mode=%s",
...
void OnDeinit(const int reason)
{
   ...
   Print("[MITEM v16.6] stopped. Reason=", reason);
}
...
void RecoverPositions()
{
   for(int i = PositionsTotal()-1; i >= 0; i--)
   {
      ...
      PrintFormat("[MITEM v16.6] RECOVERED %s ticket=%d", _Symbol, ticket);
   }
}
...
void OnTick()
{
   ...
   if(ds != g_day_start)
   {
      g_day_start = ds;
      g_daily_pnl = 0;
      g_paused = false;       // v16.6: reset daily
      g_consec_loss = 0;      // v16.6: reset daily
   }
...
void OpenTrade(int direction, string sig_type)
{
   ...
   {
      if(direction > 0)
         ok = trade.Buy(vol, _Symbol, entry, NormalizeDouble(sl, _Digits), NormalizeDouble(tp, _Digits), "MITEM_v16.6");
      else
         ok = trade.Sell(vol, _Symbol, entry, NormalizeDouble(sl, _Digits), NormalizeDouble(tp, _Digits), "MITEM_v16.6");
   }
   else
   {
   ...
   if(!ok)
   {
      PrintFormat("[MITEM v16.6] Order FAILED: %d %s", trade.ResultRetcode(), trade.ResultComment());
   ...
   PrintFormat("[MITEM v16.6] %s %s @%.5f SL=%.5f TP=%.5f Vol=%.2f Regime=%s",
...
void ClosePosition(string reason)
{
   ...
   if(!ok)
   {
      PrintFormat("[MITEM v16.6] Close FAILED: %d %s", trade.ResultRetcode(), trade.ResultComment());
      return;
   }
   ...
   // v16.6: 3-circuit-breaker (resets daily)
   ...
   PrintFormat("[MITEM v16.6] CLOSE %s R=%.3f PnL=$%.2f Equity=$%.2f",
...
void CreateDashboard()
{
   for(int i=0; i<22; i++)
   {
      dash_names[i] = "MITEM166_" + IntegerToString(i);
...
void UpdateDashboard()
{
   ...
   lines[20] = "v16.6: Wider pullback + Daily reset";
   lines[21] = "Optimized for V100";
...
{
   string name = "M166_" + tag + "_" + IntegerToString((int)t);
