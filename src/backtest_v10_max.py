"""V10 Backtest — Maximum volume within margin limits"""
...
print(f"V10: tick_val={info.trade_tick_value}, point={info.point}, dpp=${dpp:.4f}")
print(f"Min vol: {min_vol}, margin/lot: ${margin_per_lot:.2f}")
print(f"Max lots with $22.75: {int(22.75/margin_per_lot)}")
print()
...
p = {
    ...
    'max_consec_loss': 3, 'max_daily_loss_pct': 0.15,
...
for i in range(250, n1 - p['hold_bars'] - 5):
    ...
    # MAXIMUM SAFE VOLUME: use 70% of free margin
    loss_1lot = (stop_dist / info.trade_tick_size) * info.trade_tick_value
    if loss_1lot <= 0: continue
    
    max_vol_margin = (equity * 0.70) / margin_per_lot  # 70% margin usage
    max_vol_risk = (equity * 0.20) / loss_1lot  # 20% max risk per trade
    vol = min(max_vol_margin, max_vol_risk)
    ...
    if equity < 1.0: break  # Account blown
...
print('='*80)
print(f'  V10 MAX VOLUME BACKTEST — {sym}')
print(f'  Period: {start.strftime("%Y-%m-%d")} → {end.strftime("%Y-%m-%d")} (90 days)')
print(f'  Starting: ${STARTING_EQUITY:.2f} | Volume: 70% margin / 20% risk cap')
print('='*80)
print()
