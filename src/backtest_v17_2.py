"""Backtest v17.2 — V10 H1+H4 Strengthened Trend Logic"""
...
p = {
    ...
    'atr_stop': 1.6, 'atr_target': 2.5, 'hold_bars': 18,
...
def get_h4_regime(h1_time):
    for j in range(n4 - 1, -1, -1):
        if times4[j] <= h1_time:
            ...
            if j > 14:
                tr_vals = []
                for k in range(max(1, j-14), j+1):
                    tr_vals.append(max(high1[k]-low1[k] if k < n1 else 0, abs(close4[j]-close4[j-1])))
                atr4_val = np.mean(tr_vals) if tr_vals else 1.0
            else:
                atr4_val = 1.0
            sep = abs(ema_fast_4[j] - ema_mid_4[j]) / atr4_val if atr4_val > 0 else 0
...
for i in range(250, n1 - p['hold_bars'] - 5):
    ...
    trades.append({'r': r_mult, 'pnl': pnl, 'reason': exit_reason, 'regime': regime, 'sig': sig_type, 'entry': entry, 'exit': exit_price})
...
if trades:
    ...
    losses_count = sum(1 for t in trades if t['r'] <= 0)
    ...
    print('='*60)
    print('  v17.2 BACKTEST — V10 H1+H4 (90 days)')
    print('='*60)
