"""v17.3 Full Backtest — V10 H1+H4, $22.75 starting equity"""
...
for i in range(250, n1 - p['hold_bars'] - 5):
    ...
    if vol * stop_dist * (tick_val/tick_size) > equity * 0.5:
        vol = max(min_vol, round(equity * 0.3 / (stop_dist * (tick_val/tick_size)) / vol_step) * vol_step)
    ...
    trade_log.append(f"  #{len(trades):3d} {entry_dt.strftime('%m-%d %H:%M')} {sig_type:18s} {'BUY' if direction>0 else 'SELL'} @ {entry:.2f} → {exit_price:.2f} | {exit_reason:6s} | R={r_mult:+.3f} | PnL=${pnl:+.2f} | Vol={vol:.2f} | Eq=${equity:.2f}")
...
# ============ RESULTS ============
print('='*80)
print('  v17.3 BACKTEST — Volatility 10 Index — H1 Entry + H4 Regime')
...
if trades:
    ...
    max_dd = (peak - min(t['eq_after'] for t in trades)) / peak * 100 if peak > 0 else 0
    max_dd_dollar = peak - min(t['eq_after'] for t in trades)
    ...
    for t in trades:
        rg = t['regime']
        if rg not in regimes: regimes[rg] = {'n': 0, 'r': 0, 'pnl': 0}
        regimes[rg]['n'] += 1; regimes[rg]['r'] += t['r']; regimes[rg]['pnl'] += t['pnl']
    for rg, v in sorted(regimes.items()):
        print(f'    {rg:10s}: {v["n"]:3d} trades | R={v["r"]:+6.1f} | PnL=${v["pnl"]:+7.2f}')

    print()
    print('  Equity Curve (sampled):')
    print('  ' + '-'*60)
    sample_points = [0, len(trades)//4, len(trades)//2, 3*len(trades)//4, len(trades)-1]
    for idx in sample_points:
        t = trades[idx]
        print(f'    Trade #{idx+1:3d}: ${t["eq_after"]:8.2f} | {t["entry_time"].strftime("%Y-%m-%d %H:%M")} | {t["sig"]}')
    ...
    print()
    print('='*80)
    print('  FORWARD TEST PROJECTION (next 90 days)')
    print('='*80)
    print()
    ...
    # Simulate forward with compounding
    sim_eq = equity
    sim_peak = sim_eq
    sim_max_dd = 0
    for month in range(1, 13):
        sim_eq += avg_monthly_pnl * (sim_eq / STARTING_EQUITY)  # scale with equity
        if sim_eq > sim_peak: sim_peak = sim_eq
        dd = (sim_peak - sim_eq) / sim_peak * 100 if sim_peak > 0 else 0
        if dd > sim_max_dd: sim_max_dd = dd
        if month in [1, 3, 6, 12]:
            print(f'    Month {month:2d}: ${sim_eq:8.2f} (DD: {dd:.1f}%)')

    print()
    print(f'  Projected 12-month: ${sim_eq:.2f} ({(sim_eq-STARTING_EQUITY)/STARTING_EQUITY*100:+.0f}% ROI)')
    print(f'  Projected max DD:   {sim_max_dd:.1f}%')
else:
