"""v17.3 Backtest — V100 H1+H4, $22.75 equity, proper volume"""
...
print(f"V100: tick_val={info.trade_tick_value}, point={info.point}, \$/point/lot=${dpp:.4f}")
print(f"Min vol: {min_vol} → ${dpp * min_vol:.4f}/point")
print()
...
for i in range(250, n1 - p['hold_bars'] - 5):
    ...
    # Don't risk more than 10% of equity
    max_vol_safe = equity * 0.10 / loss_1lot
    vol = min(vol, max(min_vol, round(max_vol_safe / vol_step) * vol_step))
    ...
    trade_log.append(f"  #{len(trades)+1:3d} {entry_dt.strftime('%m-%d %H:%M')} {sig_type:18s} {'BUY' if direction>0 else 'SELL'} @ {entry:.2f} → {exit_price:.2f} | {exit_reason:6s} | R={r_mult:+.3f} | Vol={vol:.2f} | PnL=${pnl:+.2f} | Eq=${equity:.2f}")
...
print('='*80)
print(f'  v17.3 BACKTEST — {sym} — H1 Entry + H4 Regime')
print(f'  Period: {start.strftime("%Y-%m-%d")} → {end.strftime("%Y-%m-%d")} (90 days)')
print(f'  Starting Equity: ${STARTING_EQUITY:.2f} | Risk: {p["risk_pct"]*100:.0f}%/trade')
print('='*80)
print()
...
if trades:
    ...
    print(f'  Avg Risk/Trade:   ${np.mean([t["risk"] for t in trades]):.2f}')
    print()
    print(f'  ── Equity ──')
