"""v17.4 Backtest — V100 H1+H4, user's optimized parameters"""
...
print(f"V100: tick_val={info.trade_tick_value}, point={info.point}, DPP/lot=${dpp:.4f}")
print(f"Min vol: {min_vol}, margin/lot: ${margin_per_lot:.2f}")
print()
...
for i in range(250, n1 - p['hold_bars'] - 5):
    ...
    loss_1lot = (stop_dist / info.trade_tick_size) * info.trade_tick_value
    ...
    trade_log.append(f"  #{len(trades)+1:3d} {entry_dt.strftime('%m-%d %H:%M')} {sig_type:18s} {'BUY' if direction>0 else 'SELL'} @ {entry:.2f} → {exit_price:.2f} | {exit_reason:6s} | R={r_mult:+.3f} | Vol={vol:.1f} | Risk=${actual_risk:.2f} | PnL=${pnl:+.2f} | Eq=${equity:.2f}")
...
print('='*80)
print(f'  v17.4 BACKTEST — {sym} — H1 Entry + H4 Regime')
print(f'  Period: {start.strftime("%Y-%m-%d")} → {end.strftime("%Y-%m-%d")} (90 days)')
print(f'  Starting: ${STARTING_EQUITY:.2f} | Risk: {p["risk_pct"]*100:.2f}%/trade')
print('='*80)
print()
