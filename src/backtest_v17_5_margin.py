"""v17.5 Backtest — V100 with 70% margin volume"""
...
rates_h1 = mt5.copy_rates_range(sym, mt5.TIMEFRAME_H1, start, end)
rates_h4 = mt5.copy_rates_range(sym, mt5.TIMEFRAME_H4, start, end)
info = mt5.symbol_info(sym)

if rates_h1 is None or len(rates_h1) == 0:
    print("No data"); mt5.shutdown(); sys.exit()

dpp = info.trade_tick_value / info.trade_tick_size * info.point
min_vol = info.volume_min
vol_step = info.volume_step
point = info.point
margin_per_lot = mt5.order_calc_margin(0, sym, 1, info.ask)
if not margin_per_lot or margin_per_lot <= 0: margin_per_lot = 0.62

print(f"V100: DPP/lot=${dpp:.4f}, Margin/lot=${margin_per_lot:.2f}")
print(f"Volume: 70% margin → {int(22.75*0.70/margin_per_lot)} lots at $22.75")
print()
...
p = {
    'pullback_min': 0.45, 'pullback_max': 2.40,
    'rsi_buy_max': 64.0, 'rsi_sell_min': 36.0,
    'min_ema_sep': 0.30,
    'use_momentum': True, 'mom_lookback': 10, 'mom_min_move': 1.1,
    'mom_rsi_buy': 53.0, 'mom_rsi_sell': 47.0, 'slope_thresh': 0.40,
    'atr_target': 2.6, 'hold_bars': 16,
    'cooldown': 3, 'use_trailing': True,
    'trail_start': 1.0, 'trail_dist': 1.0,
    'use_be': True, 'be_trigger': 1.2,
    'max_consec_loss': 3, 'max_daily_loss_pct': 0.10,
    'use_session_sl': True, 'sl_quiet': 1.50, 'sl_active': 1.80,
    'quiet_start': 0, 'quiet_end': 7,
    'early_exit_hours': 5, 'early_exit_min_r': 0.20,
    'margin_pct': 0.70,  # 70% margin per trade
}
...
for i in range(250, n1 - p['hold_bars'] - 5):
    ...
    if pct > 88.0 or pct < 10.0: continue
    ...
    # MARGIN-BASED VOLUME: 70% of equity, capped at 50 lots
    margin_budget = equity * p['margin_pct']
    vol = margin_budget / margin_per_lot if margin_per_lot > 0 else 1.0
    vol = max(min_vol, round(vol / vol_step) * vol_step)
    vol = min(vol, 50.0)  # Cap at 50 lots for realistic sizing
    vol = min(vol, info.volume_max)

    actual_risk = vol * stop_dist * dpp
    ...
    trade_log.append(f"  #{len(trades)+1:3d} {entry_dt.strftime('%m-%d %H:%M')} {sig_type:18s} {'BUY' if direction>0 else 'SELL'} @ {entry:.2f} → {exit_price:.2f} | {exit_reason:6s} | R={r_mult:+.3f} | Vol={vol:.1f} | PnL=${pnl:+.2f} | Eq=${equity:.2f}")

    trades.append({'r': r_mult, 'pnl': pnl, 'reason': exit_reason, 'regime': regime, 'sig': sig_type,
                   'entry': entry, 'exit': exit_price, 'vol': vol, 'eq_after': equity, 'risk': actual_risk})
...
print('='*80)
print(f'  v17.5 BACKTEST — {sym} — 70% Margin Volume')
print(f'  Period: {start.strftime("%Y-%m-%d")} → {end.strftime("%Y-%m-%d")} (90 days)')
print(f'  Starting: ${STARTING_EQUITY:.2f} | Volume: 70% margin per trade')
print('='*80)
print()
...
if trades:
    ...
    avg_vol = np.mean([t['vol'] for t in trades])
    ...
    print(f'  ROI:              {roi:+.1f}%')
    print(f'  Avg Volume:       {avg_vol:.1f} lots')
    print(f'  Avg Risk/Trade:   ${avg_risk:.2f}')
    print(f'  Peak Equity:      ${peak:.2f}')
    print(f'  Final Equity:     ${equity:.2f}')
    print(f'  Max Drawdown:     ${peak-min_eq:.2f} ({max_dd:.1f}%)')
    ...
    for s, v in sorted(sigs.items()):
        ...
        print(f'    {s:20s}: {v["n"]:3d} trades | WR {wr_s:5.1f}% | R={v["r"]:+6.1f} | PnL=${v["pnl"]:+8.2f}')
    ...
    for r, v in sorted(reasons.items()):
        print(f'    {r:10s}: {v["n"]:3d} ({v["n"]/len(trades)*100:4.1f}%) | PnL=${v["pnl"]:+8.2f}')
    ...
    avg_monthly = avg_daily_pnl * 30
    print(f'  Avg trades/day:  {tpd:.1f}')
    print(f'  Avg P&L/day:     ${avg_daily_pnl:+.2f}')
    print(f'  Avg P&L/month:   ${avg_monthly:+.2f}')
    print()
    sim_eq = equity
    for m in [1, 3, 6, 12]:
        for _ in range(m):
            sim_eq += avg_monthly * (sim_eq / STARTING_EQUITY)
        print(f'  Month {m:2d}: ${sim_eq:8.2f} ({(sim_eq-STARTING_EQUITY)/STARTING_EQUITY*100:+.0f}% ROI)')
