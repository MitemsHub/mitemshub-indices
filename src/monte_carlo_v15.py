"""
Monte Carlo Stress Test — v15 Optimized Engine
...
# ─── BACKTEST (returns list of R-multiples) ────────────────────────
def collect_trades(m5_data, m15_data, specs, params, start_idx=250):
    ...
    for i in range(start_idx, len(close5)):
        ...
        if paused or cooldown > 0 or atr_pct < p['atr_low_pct'] or atr_pct > p['atr_high_pct']:
            continue
...
# ─── MONTE CARLO ────────────────────────────────────────────────────
def monte_carlo(r_multiples, pnl_list, n_sims=1000, initial_equity=10000.0, max_dd_pct=25.0):
    """Shuffle trade order n_sims times and collect statistics."""
    ...
    for _ in range(n_sims):
        ...
        shuffled_pnl = [pnl_list[i] for i in idx]
...
def main():
    ...
    print("=" * 70)
    print(f"  MONTE CARLO STRESS TEST — {N_SIMS} shuffled simulations per symbol")
